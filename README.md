...
## 7. 앞으로의 업데이트 계획
### 1. [리팩토링] 전역 상태관리 사용
- 지금은 사용자의 정보나 반복해서 사용해야하는 데이터들을 로컬 스토리지 혹은 props로 관리하고 있어, 내용이 많아질수록 관리가 더 어렵게 됩니다. 그래서 데이터를 효율적으로 관리하고자 전역 상태 관리 라이브러리를 사용하여 리팩토링 해보려고 합니다. 

### 2. [리팩토링] 이미지 스프라이트 기법 사용
- 현재 단일 파일로 되어있는 아이콘 이미지들을 스프라이트 이미지로 변경시켜 이미지 로딩 속도를 조금 더 올리고자 합니다.

### 3. 추가 기능 구현
- 채팅 기능 구현 및 소셜 로그인 기능을 조금 더 완성도를 높이고자 합니다.
